...
class RandomForest():
    ...
    def get_bootstrap_data(self, X, y):
        ...
        data_sets = []
        for haha in range(self.n_estimators):
...
# CAN Work here, but not in jupyter notebook
if __name__ == '__main__':
    forest_model = RandomForest(n_estimators=10)
    forest_model.fit(X=pd.read_csv('../data/data_ready.csv', index_col=0).values,
                     y=pd.read_csv('../data/labels.csv', index_col=0).values)


def Rf():
    return sklearn.ensemble.RandomForestClassifier(n_estimators=20)
